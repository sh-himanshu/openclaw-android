...
|---------|------|-----------------|
| `nodejs-lts` | Node.js LTS runtime (>= 22) + pnpm package manager | OpenClaw is a Node.js application. Node.js and pnpm are required to install it via `pnpm install -g openclaw`. LTS is used because OpenClaw requires Node >= 22.12.0 |
| `git` | Distributed version control | Some pnpm packages have git dependencies. Sub-dependencies of OpenClaw may reference packages via git URLs. Also needed if installing this repo via `git clone` |
...
- After installation, verifies Node.js >= 22 and pnpm presence. Exits on failure
...
### [5/7] OpenClaw Installation & Patching — `pnpm install` + `patches/apply-patches.sh`
...
1. Copies compatibility patches to `~/.openclaw-android/patches/`:
   - `bionic-compat.js` — Node.js runtime patches (needed during pnpm install)
...
2. Runs `pnpm install -g openclaw@latest`
...
4. `scripts/build-sharp.sh` builds the sharp native module for image processing (non-critical):
   ...
   - Runs `pnpm rebuild sharp` inside the OpenClaw directory
